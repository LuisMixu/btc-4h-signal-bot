# BTC AI-Score Signal Bot (4h) -- Einrichtung

Ein kostenloser, von TradingView unabhängiger Bot, der alle 4 Stunden prüft, ob die
BTC-AI-Score-Strategie (4h) ein neues Long/Short-Signal auf BTCUSDT (Bitget) ausgelöst hat,
und dir Entry/Stop-Loss/TP1/TP2 per Telegram schickt. Läuft auf GitHub Actions (kostenlos),
dein PC muss dafür nicht an sein.

Genau der gleiche Aufbau wie beim Gold-Signal-Bot, nur für BTC auf 4h.

Drei Schritte, alle einmalig:

## 1. Telegram-Bot erstellen (2 Minuten)

Falls du für den Gold-Bot bereits einen Telegram-Bot und Chat-ID hast, kannst du
denselben Bot/dieselbe Chat-ID wiederverwenden -- dann direkt zu Schritt 2.

1. Öffne Telegram, suche nach **@BotFather** und starte einen Chat.
2. Schick ihm `/newbot`, gib deinem Bot einen Namen und einen Nutzernamen (muss auf `bot` enden, z.B. `mein_btc_signal_bot`).
...
2. Neues **privates** Repository erstellen (z.B. `btc-4h-signal-bot`).
3. Diesen ganzen Ordner (`btc-4h-signal-bot/`, inklusive dem versteckten Unterordner
...
Zum sofortigen Testen: im Repository unter **Actions -> BTC 4h AI-Score Signal Bot -> Run workflow**
...
- Läuft komplett unabhängig vom Gold-Signal-Bot (eigenes Repository, eigene `state.json`) -- du
  kannst beide parallel laufen lassen, auch mit demselben Telegram-Bot/Chat.
